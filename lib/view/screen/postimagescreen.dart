...
import 'package:instgramclone/service/firestorage.dart';
...
Postdatascreen(this._users);
  ...
  Widget build(BuildContext context) {
   ...
   //await _storagee.storeimage(file: File(image.path), imagename: imagename,
     ...
     await addPostsToUser(docpath: _users.id,
                postimageurl:
                await _storagee.storeimage(file: File(image.path), imagename: imagename,
            refname: Consts.postimages));
   print("*********************************************************");
       },child:
            Text("Post image from camera"),),
          ElevatedButton(onPressed: ()async{
            final image=await _picker.pickImage(source: ImageSource.gallery);
          },child:
          Text("Post image from Gallery"),),
        ],
      )),);
  }
}
...
DocumentReference ref= FirebaseFirestore.instance
    .collection(Consts.users).doc(docpath);
FirebaseFirestore.instance.runTransaction((transaction)async{
  await transaction.update(ref,{"Post":[
      {"postimageurl":postimageurl,"comments":["ffffffffff","uuuuuuu"]},

      ]});
});}

Future<void>addPostsToUser({@required String? docpath,@required String? postimageurl})async{
