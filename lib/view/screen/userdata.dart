...
import '../../service/firestorage.dart';
class Userdata extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    XFile? imagee;
    String?imageename;
    return Scaffold(appBar: AppBar(title: Text("Enter your data"),),
      ...
      child: Column(children: [
       ...
       texetfield("First name",firstnamecontroller),
        ...
        Padding(
          padding: const EdgeInsets.only(top: 50),
          child: ElevatedButton(onPressed: ()async{
         await _storage.storeimage(file: File(imagee!.path),
             imagename: imageename!,
             refname: Consts.profileimages);
         print("***************************************************************");
         await addUser(email: emaill,
  ...
  Widget texetfield(String text,TextEditingController _controller){
    return Padding(
      padding: const EdgeInsets.only(top: 50),
      child: TextField(decoration: InputDecoration(hintText: text)
        ,controller: _controller,

      ),
    );}
}
Future<void> addUser({String? firstname,String?secondname,String?age,String?id,
  String?email,String?career,
String? imageuri})
async{
